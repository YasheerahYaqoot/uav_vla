...
class VLA():
    ...
    def __init__(self, command, with_tsp, without_tsp):
        
        self.with_tsp = with_tsp
        self.without_tsp = without_tsp
        
        #---- deleting existing files
        if self.without_tsp:
            files_to_delete = ['result_coordinates_no_tsp.txt']
            for file_name in files_to_delete:
                if os.path.exists(file_name):
                    os.remove(file_name)
        if self.with_tsp:
            files_to_delete = ['result_coordinates_tsp.txt']
            for file_name in files_to_delete:
                if os.path.exists(file_name):
                    os.remove(file_name)

        if self.without_tsp:
            self.result_file = 'result_coordinates_no_tsp.txt'
        elif self.with_tsp:
            self.result_file = 'result_coordinates_tsp.txt'

        #---- making identified new data directory
        if self.without_tsp:
            #---- making mission directory
            mission_directory = 'created_missions_no_tsp'
            if os.path.exists(mission_directory):
                shutil.rmtree(mission_directory)  
            os.makedirs(mission_directory)
            identified_new_data_directory = 'identified_new_data_no_tsp'
            ...
            mission_directory = 'created_missions_tsp'
            if os.path.exists(mission_directory):
                shutil.rmtree(mission_directory)  
            os.makedirs(mission_directory)
            identified_new_data_directory = 'identified_new_data_tsp'
            if os.path.exists(identified_new_data_directory):
                shutil.rmtree(identified_new_data_directory)  
            os.makedirs(identified_new_data_directory) 
        ...
        self.number_of_samples = 30 #len(os.listdir('/VLM_Drone/dataset_images'))
    ...
    def set_templates(self):
        ...
        Example output:
        {{
            "object_types": ["red tripod stands", "three legs of each red tripod stand", "each three legs of red tripod stands", "three legs end-point of each red tripod stands", "center of the X on yellow objects", "chair", "table with a red H"],
        }}
        """
    ...
    def find_objects(self, json_input, example_objects):
        ...
        with open(self.result_file, 'a') as file:
            ...
            for i in range(1, self.number_of_samples+1):
                ...
                if self.without_tsp:
                    ...
                    draw_dots_and_lines_on_image(f'benchmark-UAV-VLPA-nano-30/images/{i}.jpg', parsed_points, output_path=f'identified_new_data_no_tsp/identified{i}.png')
                    ...
                    print('\n')
                    result_coordinates = outdoor_VLA_utils.recalculate_coordinates(optimized_coordinates, image_number, coordinates_dict)
                    draw_dots_and_lines_on_image(f'benchmark-UAV-VLPA-nano-30/images/{i}.jpg', optimized_coordinates, output_path=f'identified_new_data_tsp/identified{i}.png')
                    self.list_of_the_resulted_coordinates_percentage.append(parsed_points)
        ...
        if self.without_tsp:
            return json.dumps(result_coordinates), self.list_of_the_resulted_coordinates_percentage, self.list_of_the_resulted_coordinates_lat_lon
        elif self.with_tsp:
    ...
    def generate_drone_mission(self):
        ...
        if self.without_tsp:
            objects_json, list_of_the_resulted_coordinates_percentage, list_of_the_resulted_coordinates_lat_lon = self.find_objects(object_types_json, self.example_objects)
        elif self.with_tsp:
